...
# Update appointment status (for staff: Accept, Cancel, Complete, Pending)
@login_required(login_url='login')
def update_status(request, appointment_id):
    if not (request.user.is_staff or request.user.is_superuser):
        messages.error(request, 'Only studio admins can update appointment status.')
...
# Cancel appointment (for regular clients)
@login_required(login_url='login')
